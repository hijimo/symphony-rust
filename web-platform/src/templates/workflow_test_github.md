...
<!-- [EXPERIMENTAL] 按需补写 E2E/集成测试 — 观察效果中，可能后续调整或移除 -->

Evaluate whether this PR needs new E2E or integration tests:

| Condition | Action |
|-----------|--------|
| PR adds a new user-facing feature entry (page, API endpoint, CLI command) | Write E2E test |
| PR modifies cross-module data flow (auth, payment, state machine) | Write integration test |
| Acceptance Criteria describe end-to-end behavior with no existing E2E coverage | Write E2E test |
| PR is a small bugfix/refactor with existing test coverage | Skip — run existing tests only |

If you decide to write E2E/integration tests, commit them to the PR branch before producing the Test Report.
...
- Do not implement features or fix bugs — only write tests, commit them to the PR branch, and report
<!-- [EXPERIMENTAL] 允许 test agent 提交测试代码到 PR 分支 — 观察效果中 -->
- You MAY commit new test files or add test cases to existing test files on the PR branch
